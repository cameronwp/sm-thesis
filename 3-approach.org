...
cases, both related to a change in the /execution space/ that results from the variable-delay to
fixed-delay STNU transformation. In one case, we are allowed to imagine that contingent events were
assigned despite never observing them (see Section [[sec:recording-vdc-ctg]]). In the other, we use the
observation to optimistically rewrite the variable-delay STNU in an attempt to shorten the overall
makespan (see Section [[sec:optimistic]]).
