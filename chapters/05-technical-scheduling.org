...
:startup: content
:END:
** DONE consistency with "noop"
- State "DONE"       from "TODO"       [2023-08-13 Sun 21:52]
is it =noop= or =no-op= or $\mathit{noop}$ or /noop/ or /no-op/?
** DONE consistency with capitalization and italics of Scheduler, Delay Scheduler, Dispatcher, Driver
- State "DONE"       from "TODO"       [2023-08-13 Sun 21:52]
** DONE we need an =updateSchedule= algo defined in the execution strategy section
- State "DONE"       from "TODO"       [2023-08-13 Sun 21:52]
...
** DONE fix:observations is weird. fix notation, caption
- State "DONE"       from "TODO"       [2023-08-13 Sun 21:52]
** DONE RTED defn is missing info in the scheduler section. also check it is accurate wrt code
- State "DONE"       from "TODO"       [2023-08-13 Sun 21:52]
** extra content 1
...
/AllMax/ in favor of their strongest replacements with respect to $Z$. We include pseudo-code of the
real-time update step in Algorithm [[alg:fast-ex-update]].
...
[[sec:vdc-to-fdc]]. $S$ and $x_{c}$ refer to a variable-delay STNU and an uncontrollable event within
...
1. uncontrollable events with infinite observation delay, and
2. uncontrollable events that are observed outside their bounds in $S'$
...
[cite:@Hunsberger2016; p.129] that guarantees that $t$ is greater than the current time.
...
#+caption: An STNU roughly representing the installation and test of repeater antennas. Each row represents a single rover. The episode durations are representative of the bounds used in simulation. Circles with solid lines represent controllable events, while dashed lines are uncontrollable events.
